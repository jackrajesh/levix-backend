...
def fuzzy_match_with_score(query: str, db: Session, shop_id: str) -> Tuple[Optional[models.InventoryItem], float]:
    """
    STRICT MATCH MODE:
    ...
    items = db.query(models.InventoryItem).filter(models.InventoryItem.shop_id == shop_id).all()
...
def fuzzy_match_product(query: str, db: Session, shop_id: str) -> Optional[models.InventoryItem]:
...
def get_product_status(item: models.InventoryItem) -> str:
    """Standardized product status based on quantity only."""
    if item.quantity <= 0:
        return 'out_of_stock'
...
def sync_stock_status(item: models.InventoryItem, db: Session):
    """Legacy status sync removed for Retail POS."""
    return

def handle_low_stock_log(item: models.InventoryItem, db: Session):
    """Legacy low stock logging removed for Retail POS."""
    return
